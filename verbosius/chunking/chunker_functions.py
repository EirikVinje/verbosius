...
def chunk_data(n_chunks_per_mix, n_classes, split_ind_input, texts, labels, dataset, seed):
    ...
    for i in range(n_chunks_per_mix):
        ...
        if dataset.n_classes >= 3:
            split_label_orig = dataset.load_orig_labels()   # load all original chunks
            split_label_orig = split_label_orig[split_ind][:, 1]  # extract specific indices for given chunk
...
def chunk_data_multiclass_supersample(dataset, n_chunks_per_mix : int, chunk_size : int, shuffle : bool = True, seed : int = 42):  
    ...
    return train_x, train_y, train_y_orig, n_classes


def datachunker_amazon(data, chunk_size, chunk_amount, orig_y, seed):
    ...
    with tqdm(total=chunk_amount, disable=False) as bar:

        bar.set_description("Processing chunk 1 of {}:".format(chunk_amount))
        ...
        for i in range(chunk_amount):
            ...
            bar.set_description("Processing chunk {} of {}".format(i+1, chunk_amount))
            bar.update(1)

            all_chunks.append(i_x)
