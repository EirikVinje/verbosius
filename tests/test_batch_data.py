...
#     batch_data_multi = datasource.batch_data_multiclass(dataset = ds,
#                                             n_batches_per_mix=2,
#                                             batch_size=batch_size,
#                                             path=path,
#                                             test=True,
...
def test_batch_train_test_splits():
    path = (((Path(__file__).parent).parent).parent).parent / "data" / "verbosius" / "imdb"
    ...
    # since use_test_set == True, and no test_batch_size has been set, the test set will be the same sizes as the train set
    batch_data = datasource.batch_data_multiclass(dataset = ds,
                                                    n_batches_per_mix=2,
                                                    batch_size=batch_size,
                                                    path=path,
                                                    test=True,
    ...
    assert len(batch_data[0]) == 2, "train_x 1"
    assert len(batch_data[1]) == 2, "train_y 1"
    assert len(batch_data[2]) == 2, "test_x 1"
    assert len(batch_data[3]) == 2, "test_y 1"
    assert len(batch_data[0][0]) == 12500, "train_x 2"
    assert len(batch_data[1][0]) == 12500, "train_y 2"
    assert len(batch_data[2][0]) == 12500, "test_x 2"
    assert len(batch_data[3][0]) == 12500, "test_y 2"

    # since use_test_set == True, and test_batch_size has been set, the test set will now have a different size
    batch_data = datasource.batch_data_multiclass(dataset = ds,
                                                    n_batches_per_mix=2,
                                                    batch_size=batch_size,
                                                    path=path,
    ...
    assert len(batch_data[0][0]) == 12500, "train_x 2"
    assert len(batch_data[1][0]) == 12500, "train_y 2"
    assert len(batch_data[2][0]) == 1000, "test_x 2"
    assert len(batch_data[3][0]) == 1000, "test_y 2"

    # since test_Size is set, train and test will now have sizes depending on this value
    test_size = 0.2
    batch_data = datasource.batch_data_multiclass(dataset = ds,
                                                    n_batches_per_mix=2,
                                                    batch_size=batch_size,
    ...
    test_mult = batch_size*test_size
    assert len(batch_data[0]) == 2, "train_x 1"
    assert len(batch_data[1]) == 2, "train_y 1"
    assert len(batch_data[2]) == 2, "test_x 1"
    assert len(batch_data[3]) == 2, "test_y 1"
    assert len(batch_data[0][0]) == batch_size - test_mult, "train_x 2"
    assert len(batch_data[1][0]) == batch_size - test_mult, "train_y 2"
    assert len(batch_data[2][0]) == test_mult, "test_x 2"
    assert len(batch_data[3][0]) == test_mult, "test_y 2"

    # since test_batches_per_mix is set the test set will now only have one batch
    batch_data = datasource.batch_data_multiclass(dataset = ds,
                                                    n_batches_per_mix=2,
                                                    batch_size=batch_size,
                                                    path=path,
    ...
    assert len(batch_data[0]) == 2, "train_x 1"
    assert len(batch_data[1]) == 2, "train_y 1"
    assert len(batch_data[2]) == 1, "test_x 1"
    assert len(batch_data[3]) == 1, "test_y 1"
    assert len(batch_data[0][0]) == batch_size - test_mult, "train_x 2"
    assert len(batch_data[1][0]) == batch_size - test_mult, "train_y 2"
    assert len(batch_data[2][0]) == test_mult, "test_x 2"
    assert len(batch_data[3][0]) == test_mult, "test_y 2"
